...
# The listed values describe an individual flow.  A multiplier makes the
# aggregate demo exceed the 10 Mbps normal link, so congestion visibly changes
# the throughput chart and the three-state comparison table.
DEMO_LOAD_MULTIPLIER = 2.2
...
def seed_demo_traffic() -> list[dict[str, Any]]:
    ...
    for label, sample_rate, classification in classified_demo:
        state.add_traffic(
            ...
            requested_mbps=round(sample_rate * DEMO_LOAD_MULTIPLIER, 3),
        )
...
def throughput_comparison() -> dict[str, Any]:
    """Aggregate requested traffic by tier for each demo state shown in the UI."""
    traffic = state.get_traffic_list()
    prepared = [
        {
            **item,
    ...
    tiers = ("emergency", "critical_sensor", "video", "file", "background")
    scenarios = (
        ("Normal link", False, True),
        ("Congested · fair allocation", True, False),
        ("Congested · semantic routing", True, True),
    )
    results = []
    for label, congested, semantic in scenarios:
        allocation = routing_service.allocate_bandwidth(prepared, congested, semantic)
        by_tier = {tier: 0.0 for tier in tiers}
        for item in prepared:
            by_tier[item["type"]] += allocation[item["id"]]
        results.append(
            {
                "label": label,
                "bandwidth_mbps": 2.0 if congested else 10.0,
                "tiers": {tier: round(rate, 3) for tier, rate in by_tier.items()},
                "total_mbps": round(sum(by_tier.values()), 3),
            }
        )
