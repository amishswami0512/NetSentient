...
class StateService:
    ...
    # -- traffic -----------------------------------------------------
    def add_traffic(
        self, traffic_type: str, label: str, metadata: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    def clear_scanned_traffic(self) -> None:
        """Remove the previous local-scan snapshot without touching demo/manual flows."""
        self._traffic = {
            traffic_id: entry
            for traffic_id, entry in self._traffic.items()
            if entry.get("source_kind") != "local_scan"
        }
